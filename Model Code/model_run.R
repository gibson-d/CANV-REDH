...
model_1 <- nimbleCode( {
  ...
  for(k in 1:n.species){
    beta.winter[k] ~  dlogis(0, 1)       # Intercept-adjustment for post-harvest releases
  ...
  for (t in 1:n.occasions){
    for(k in 1:n.species){
      for(l in 1:n.sex){
        ...
        kappa[3,k,l,t] <- ( 1 - s.winter[2,k,l,t]) * (haz.kappa[3,k,l,t]/ winter_risk[2,k,l,t])        # Indirect Post-Breeding rate
        f[3,k,l,t] <- kappa[3,k,l,t] * r[t] * (1 - cr)
        
        kappa[4,k,l,t] <- ( 1 - s.winter[2,k,l,t]) * (haz.kappa[4,k,l,t]/ winter_risk[2,k,l,t])        # Indirect Post-Harvest rate
      ...
      for (t in 1:(n.occasions-1)){
        ...
        for (j in (t+1)){
          ...
          pr.f[2,l,k,t,j] <- s.winter[2,l,k,t] * s.summer[2,l,k,t] * f[3,l,k,j]  # survived the winter and summer after banding, reported following fall
            pr.w[l,k,t,j] <-  s.summer[1,l,k,t]  * f[4,l,k,j]                    # survived summer, reported in fall (HY)
        }
      }
  ...
  for(k in 1:n.species){
    for(j in 1:3){
      beta.np[k,j] ~ dnorm(0, 0.01) # Regression Coefficients
    }
    for(l in 1:n.sex){
      nu.theta[k,l] ~ dlogis(0,1)   # Intercept - Age Ratio
    }
    nu.omega[k] ~dlogis(0,1)        # Intercept - Sex Ratio
    sig.beta[k] ~ T(dt(0, pow(2.5, -2), 1),0,)
    for(i in 1:K){
      beta.spline[k,i] ~ dnorm(0, sd = sig.beta[k])  # P-spline differencing coefficients
    }
    for(t in 1:n.occasions){
    ...
    # Prediction for the p-spline model
    for(t in 1:n.occasions){
      for(l in 1:n.sex){
  ...
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
  # Spatial random effects via a Icar distribution
  ...
  for (t in 1:n.occasions) {
    ...
    # Generate Covariate for survival model
...
out <- clusterEvalQ(cl_ipm2, {
  ...
  CmodelMCMC$run(5000, thin = 2, thin2 = 2, nburnin = 2500)
